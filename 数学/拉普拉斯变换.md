...
2. \textbf{拉普拉斯变换的线性性质}
   \begin{itemize}
       \item 使用拉普拉斯变换的线性性质，求解以下表达式的拉普拉斯变换：
       \begin{enumerate}
           \item \( f(t) = 3e^{-2t} + 4\sin(3t) \)
           \item \( f(t) = t^2 - 5\cos(4t) \)
       \end{enumerate}
   \end{itemize}

3. \textbf{单位阶跃函数的拉普拉斯变换}
   \begin{itemize}
       \item 计算以下函数的拉普拉斯变换：
       \begin{enumerate}
           \item \( f(t) = u(t-2) \cdot e^{-3(t-2)} \), 其中 \(u(t)\) 是单位阶跃函数
           \item \( f(t) = (t-1)u(t-1) \)
       \end{enumerate}
   \end{itemize}
...
5. \textbf{反拉普拉斯变换}
   \begin{itemize}
       \item 求解以下拉普拉斯变换的原函数：
       \begin{enumerate}
           \item \( F(s) = \frac{2}{s^2 + 4s + 5} \)
           \item \( F(s) = \frac{s + 3}{s^2 + 2s + 2} \)
           \item \( F(s) = \frac{5s}{s^2 + 9} \)
       \end{enumerate}
   \end{itemize}

6. \textbf{应用题：解微分方程}
   \begin{itemize}
       \item 使用拉普拉斯变换求解以下微分方程的解：
       \begin{enumerate}
           \item \( y'' + 4y' + 4y = 0 \), 初始条件 \(y(0) = 1\), \(y'(0) = 0\)
           \item \( y'' + 2y' + 5y = \sin(t) \), 初始条件 \(y(0) = 0\), \(y'(0) = 1\)
       \end{enumerate}
   \end{itemize}
